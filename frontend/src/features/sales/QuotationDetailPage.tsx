...
export function QuotationDetailPage() {
  ...
  async function handleShareQuotation() {
    ...
    try {
      ...
      // particular). window.open() is wrapped in its own try/catch and treated as failed on EITHER
      // a null return (the usual popup-blocked signal — a real possibility here since this call is
      // already several `await`s removed from the original click) OR a thrown exception —
      // confirmed on a real device this can throw instead of returning null for a blob: URL. Either
      // way this falls back to the forced download, the one path proven to actually work everywhere.
